...
this request will update said certificate using its payload. If the payload includes an `snis`
pseudo-attribute, then the list of associated snis associated with the certificate will also
be updated.

If the certificate for the provided id does not exist, then this request will create a new
certificate using its payload. If the payload includes an `snis` pseudo-attribute, then it will
also create a list SNIs associated to the new certificate.

If no SNI with the provided sni name could be found, then this request will first create a new
certificate using its payload. Then create a single SNI associated to it using the provide SNI name.
Note that if the payload does include the `snis` pseudo-attribute and said attribute does not include
the provided SNI name, an error will be produced.
...
If an SNI exists for the given UUID or name, then this request will update it
using the payload. If the SNI does not exist, this request will create a new SNI
using the given UUID or name and the payload.
